import { NextRequest, NextResponse } from 'next/server';
import { createClient } from '@supabase/supabase-js';

// Create a Supabase client with the service role key
const supabase = createClient(
  process.env.NEXT_PUBLIC_SUPABASE_URL!,
  process.env.SUPABASE_SERVICE_ROLE_KEY!,
  {
    auth: {
      persistSession: false,
    },
    db: {
      schema: 'public',
    },
  }
);

// Helper function to check if time is valid
function isValidTime(time: string): boolean {
  if (!time) return false;
  return new Date(time).toString() !== 'Invalid Date';
}

// Helper function to check for overlapping time-off
async function checkForOverlaps(
  therapistId: string,
  startTime: string,
  endTime: string,
  excludeId?: string
): Promise<boolean> {
  const query = supabase
    .from('time_off')
    .select('*')
    .eq('therapist_id', therapistId);

  if (excludeId) {
    query.neq('id', excludeId);
  }

  const { data: existingSlots, error } = await query;

  if (error || !existingSlots) {
    console.error('Error checking for overlapping slots:', error);
    return false; // Assume no overlap if there's an error
  }

  const newStartTime = new Date(startTime);
  const newEndTime = new Date(endTime);

  // Check if any existing slot overlaps with the new slot
  return existingSlots.some((slot) => {
    const slotStartTime = new Date(slot.start_time);
    const slotEndTime = new Date(slot.end_time);
    
    // Check for overlap
    return (
      (newStartTime < slotEndTime && newStartTime >= slotStartTime) ||
      (slotStartTime < newEndTime && slotStartTime >= newStartTime)
    );
  });
}

// POST /api/time-off
// Creates a new time-off block
export async function POST(request: NextRequest) {
  try {
    const body = await request.json();
    const { therapist_id, start_time, end_time, reason, recurrence } = body;

    // Validate required fields
    if (!therapist_id || !start_time || !end_time) {
      return NextResponse.json(
        { error: 'Missing required fields: therapist_id, start_time, end_time' },
        { status: 400 }
      );
    }

    // Validate times
    if (!isValidTime(start_time) || !isValidTime(end_time)) {
      return NextResponse.json(
        { error: 'Invalid time format. Expected ISO-8601 format' },
        { status: 400 }
      );
    }

    // Validate end time is after start time
    if (new Date(end_time) <= new Date(start_time)) {
      return NextResponse.json(
        { error: 'End time must be after start time' },
        { status: 400 }
      );
    }

    // Check for overlapping time-off blocks
    const hasOverlap = await checkForOverlaps(
      therapist_id,
      start_time,
      end_time
    );

    // For time-off, we allow overlaps (therapist might have multiple reasons)
    // but we'll warn the client
    let warningMessage = null;
    if (hasOverlap) {
      warningMessage = 'This time-off block overlaps with another time-off block';
    }

    // Insert time-off block
    const { data, error } = await supabase
      .from('time_off')
      .insert([{ therapist_id, start_time, end_time, reason, recurrence }])
      .select()
      .single();

    if (error) {
      console.error('Error creating time-off block:', error);
      return NextResponse.json(
        { error: error.message },
        { status: 500 }
      );
    }

    return NextResponse.json({
      success: true,
      time_off_id: data.id,
      warning: warningMessage
    });
  } catch (error) {
    console.error('Unexpected error:', error);
    return NextResponse.json(
      { error: 'An unexpected error occurred' },
      { status: 500 }
    );
  }
}

// DELETE /api/time-off/:id
// Deletes a time-off block
export async function DELETE(request: NextRequest) {
  try {
    // Extract ID from path
    const url = new URL(request.url);
    const pathParts = url.pathname.split('/');
    const id = pathParts[pathParts.length - 1];

    if (!id || id === 'time-off') {
      return NextResponse.json(
        { error: 'Time-off ID is required in the path' },
        { status: 400 }
      );
    }

    // Check if the time-off block exists
    const { data: existingTimeOff, error: fetchError } = await supabase
      .from('time_off')
      .select('id')
      .eq('id', id)
      .single();

    if (fetchError || !existingTimeOff) {
      return NextResponse.json(
        { error: 'Time-off block not found' },
        { status: 404 }
      );
    }

    // Delete time-off block
    const { error: deleteError } = await supabase
      .from('time_off')
      .delete()
      .eq('id', id);

    if (deleteError) {
      console.error('Error deleting time-off block:', deleteError);
      return NextResponse.json(
        { error: deleteError.message },
        { status: 500 }
      );
    }

    return NextResponse.json({ success: true });
  } catch (error) {
    console.error('Unexpected error:', error);
    return NextResponse.json(
      { error: 'An unexpected error occurred' },
      { status: 500 }
    );
  }
} 